...
## Version Duplicate (Public API — api.dealhub.io)

Docs: https://developers.dealhub.io/docs/version-api-overview

### List versions
```
GET https://api.dealhub.io/api/v1/versions
Authorization: Bearer {token}
```
Returns array of versions with `version_id`, `name`, `status` (ACTIVE | DRAFT | DEACTIVATED).

### Duplicate version
```
POST https://api.dealhub.io/api/v2/version/duplicate
Authorization: Bearer {token}
...
```
- `version_id`: numeric string ID of the **source** version (typically ACTIVE)
- `new_version_name`: required — name for the new DRAFT
- `comment`: optional
- `to_account_instance`: optional — cross-environment promotion (Sandbox → Production)
- Response: `{ "request_id": "..." }` — **async**, must poll to confirm completion
- **Cannot edit the new DRAFT until duplication completes**

### Activate version
```
POST https://api.dealhub.io/api/v1/version/activate
Authorization: Bearer {token}
```
Body: `{ "version_id": "..." }` — Response: `{ "request_id": "..." }` — async
- Previous ACTIVE auto-transitions to DEACTIVATED
- Activation is irreversible from this state

### Poll async status
```
GET https://api.dealhub.io/api/AsyncRequest/{request_id}
Authorization: Bearer {token}
```

### Auth note
These are **public API endpoints** (api.dealhub.io) using a long-lived Bearer token, NOT the internal admin cookie-based API. Bearer token is generated by a CPQ Admin.
